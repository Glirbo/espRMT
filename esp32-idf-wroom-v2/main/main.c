/*
 * ESP32 Robotic Arm Controller - ESP32-WROOM Edition (6 Motors)
 * Receives binary position commands via UART and controls stepper motors via RMT
 *
 * Architecture (ESP32-WROOM):
 *   - UART0: Data channel for motion commands (29-byte binary) and position queries ('P')
 *            Note: Logging also outputs to UART0 (WROOM has no separate USB-JTAG)
 *   - RMT: Hardware-timed pulse generation on 6 motors (GPIO12-15,25-27 STEP, GPIO14,16-18,32-33 DIR)
 ...
 *   - Reduced logging verbosity (DEBUG level) to minimize UART traffic
 *   - Position responses have NO log prefix for clean UART parsing
 */
...
// Per-motor gear ratios: Motors 0 and 5 = 1:10, Motors 1-4 = 1:50
static const float motor_gear_ratios[NUM_MOTORS] = {
    10.0f,  // Motor 0: 1:10 gearing
    50.0f,  // Motor 1: 1:50 gearing
    50.0f,  // Motor 2: 1:50 gearing
    50.0f,  // Motor 3: 1:50 gearing
    50.0f,  // Motor 4: 1:50 gearing
    10.0f   // Motor 5: 1:10 gearing
};

// Per-motor steps per degree (calculated from STEPS_PER_REV * gear_ratio / 360)
static const float steps_per_degree[NUM_MOTORS] = {
    111.11f,  // Motor 0: (4000 * 10) / 360 = 111.11 steps/degree
    555.56f,  // Motor 1: (4000 * 50) / 360 = 555.56 steps/degree
    555.56f,  // Motor 2: (4000 * 50) / 360 = 555.56 steps/degree
    555.56f,  // Motor 3: (4000 * 50) / 360 = 555.56 steps/degree
    555.56f,  // Motor 4: (4000 * 50) / 360 = 555.56 steps/degree
    111.11f   // Motor 5: (4000 * 10) / 360 = 111.11 steps/degree
};

// Motion Limits
#define MAX_ANGULAR_VELOCITY_DEG_PER_SEC  60.0f  // Maximum 60°/sec
// MIN_PERIOD_US must be calculated for the highest step rate (motor with highest steps/degree)
// Max steps/degree = 555.56 (motors 1-4 with 1:50 gearing)
// At 60°/sec: 60 * 555.56 = 33,333 steps/sec -> period = 30μs
#define MIN_PERIOD_US ((1000000.0f) / (MAX_ANGULAR_VELOCITY_DEG_PER_SEC * 555.56f))
// = 30μs minimum period at 60°/sec (33,333 steps/sec max)

// RMT Configuration
#define RMT_CLK_DIV     80      // 80MHz / 80 = 1MHz (1 tick = 1μs)
#define PULSE_WIDTH_US  5       // Pulse width in microseconds

// GPIO Pin Definitions for ESP32-WROOM
// Safe output pins: GPIO 12-19, 21-23, 25-27, 32-33
...
// 3.3V Power Output Pin
#define POWER_3V3_PIN   GPIO_NUM_21  // Always high for external power
...
// Per-motor capacity at 3072 items:
//   Motors 1-4 (1:50, 555.56 steps/°): 5.5° max in 50ms (1,667 steps @ 60°/s)
//   Motors 0,5 (1:10, 111.11 steps/°): 27.6° max in 50ms (333 steps @ 60°/s)
// Buffer sized for ESP32-WROOM memory constraints (costs ~74KB RAM for 6 motors)
// WROOM has 520KB SRAM total - must leave room for system + stack + heap
#define MAX_RMT_ITEMS 3072  // Adequate for 60°/s @ 50ms updates
                            // Larger moves will be automatically clamped with warning
...
void setup_gpio(void)
{
    ...
    // Configure GPIO21 as 3.3V power output (always high)
    gpio_set_direction(POWER_3V3_PIN, GPIO_MODE_OUTPUT);
    gpio_set_level(POWER_3V3_PIN, 1);
    ESP_LOGI(TAG, "✓ GPIO%d configured as 3.3V power output (always HIGH)", POWER_3V3_PIN);
    ESP_LOGI(TAG, "");
}
...
void move_motor_rmt(int motor_index, uint32_t steps, bool direction, uint32_t time_ms)
{
    ...
    if (period_us < (uint32_t)MIN_PERIOD_US) {
        ...
        float actual_velocity = (1000000.0f / period_us) / steps_per_degree[motor_index];
...
void send_current_position(void)
{
    ...
    // To debug position queries, change to ESP_LOGD and enable DEBUG level logging
}
...
void process_command(MotionCommand *cmd)
{
    ...
    for (int i = 0; i < NUM_MOTORS; i++) {
        ...
        // Convert to steps using per-motor steps_per_degree
        int32_t steps_delta = (int32_t)roundf(angle_delta * steps_per_degree[i]);
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "╔════════════════════════════════════════════════════════╗");
    ESP_LOGI(TAG, "║                                                        ║");
    ESP_LOGI(TAG, "║   ESP32-WROOM ROBOTIC ARM CONTROLLER v2.0              ║");
    ESP_LOGI(TAG, "║   Per-Motor Gearing │ 60°/sec Max Velocity            ║");
    ESP_LOGI(TAG, "║                                                        ║");
    ESP_LOGI(TAG, "╚════════════════════════════════════════════════════════╝");
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "Motor Configuration (V2 - Per-Motor Gearing):");
    ESP_LOGI(TAG, "  Steps per revolution: %.0f (no microstepping)", STEPS_PER_REV);
    ESP_LOGI(TAG, "  Per-motor gear ratios and steps/degree:");
    for (int i = 0; i < NUM_MOTORS; i++) {
        float max_steps_per_sec = MAX_ANGULAR_VELOCITY_DEG_PER_SEC * steps_per_degree[i];
        ESP_LOGI(TAG, "    Motor %d: 1:%.0f gearing │ %.2f steps/° │ max %.0f steps/s @ 60°/s",
                 i, motor_gear_ratios[i], steps_per_degree[i], max_steps_per_sec);
    }
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "Motion Limits:");
    ESP_LOGI(TAG, "  Max angular velocity: %.1f°/sec (all motors)", MAX_ANGULAR_VELOCITY_DEG_PER_SEC);
    ESP_LOGI(TAG, "  Max step rate:        33,333 steps/sec (motors 1-4)");
    ESP_LOGI(TAG, "  Max step rate:        6,667 steps/sec (motors 0,5)");
    ESP_LOGI(TAG, "  Min pulse period:     %.0fμs (based on highest step rate)", MIN_PERIOD_US);
    ESP_LOGI(TAG, "  Buffer size:          %d items per motor", MAX_RMT_ITEMS);
    ESP_LOGI(TAG, "    Motors 1-4 (1:50): %.1f° max in 50ms", (float)MAX_RMT_ITEMS / 555.56f);
    ESP_LOGI(TAG, "    Motors 0,5 (1:10): %.1f° max in 50ms", (float)MAX_RMT_ITEMS / 111.11f);
    ESP_LOGI(TAG, "");

    // Initialize hardware
    setup_gpio();   // Configure GPIO21 as 3.3V power output
    setup_uart();
    setup_rmt();
